...
# Deploy Stack
aws cloudformation deploy --no-fail-on-empty-changeset --capabilities CAPABILITY_NAMED_IAM --template-file api-gateway_iam_auth.yml --stack-name ${CF_STACK_NAME} --parameter-overrides ${CF_PARAMETER_OVERRIDE} --tags "ApplicationName=$ApplicationName" "CostReference=$CostReference" "Creator=$Creator" "CustomerContactMail=$CustomerContactMail" "Environment=$Environment" "TicketReference=$TicketReference"
